...
def getProteinDict(extracted_sent, stanford_tokens):
    mydict = dict()
    for prot_dict in extracted_sent["ProteinDetails"]:  # extracted_sent["ProteinDetails"] is a list of dicts. There may be multiple proteins detected in a single sentence
        if prot_dict["protName"] in mydict: # Each dict corresponds to a single unique protein (even though that protein may occur multiple times in the same sentence)
...
def save_problem_pmids():
    bad_chars_dict = "NonOA-BadChars.p"
