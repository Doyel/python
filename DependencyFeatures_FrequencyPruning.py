...
# python DependencyFeatures_FrequencyPruning.py RNAi Dependency_Features.json ./../datafiles/datafiles_WithWeights_BaseLine2_Aug31 DepParseFeatures_FrequencyCnts.json 50 -c
# python DependencyFeatures_FrequencyPruning.py omission Dependency_Features.json ./../datafiles/datafiles_WithWeights_BaseLine2_Aug31 DepParseFeatures_FrequencyCnts.json 50
# python DependencyFeatures_FrequencyPruning.py KO Dependency_Features.json ./../datafiles/datafiles_WithWeights_BaseLine2_Aug31 DepParseFeatures_FrequencyCnts.json 50
...
def main_body():
    ...
    parser = argparse.ArgumentParser(prog='DependencyFeatures_FrequencyPruning', usage='DependencyFeatures_FrequencyPruning.py <label> <depParse_features> <passage_dict_loc> <outputJSON> <numfeatures> [-c]', description='Script to perform frequency analysis of dependency parse features')
    ...
    if args.c:
        print "Performing a clean build of the Dependency features frequency counts json file"
        silentremove(os.path.join(json_location, args.depParseFreqJSON))
    ...
    for fname in ["train_passage_dict.json", "openaccess_passage_dict.json"]:
        ...
        for pmid in passage_dict:
            print "PMID: ", pmid
            article_level_features = [] # Stores features extracted from all instances corresponding to all proteins in an article
            for myclass in passage_dict[pmid][label]:
                for prot in passage_dict[pmid][label][myclass]:  # prot is the DatumKB protein. It could be a synonym of the actual protein mentioned in the article
                    ...
                    for contg_psg_dict in passage_dict[pmid][label][myclass][prot]["passageDetails"]:  # passage_dict[pmid][label]["Pos"][prot]["passageDetails"] is a list of dicts.
                        instance_level_features = []    # Stores features extracted from a single instance corresponding to a single protein
                        sent_ids = getSentenceIds(contg_psg_dict, fname)  # contg_psg_dict represents one training instance
                        for sentid in sent_ids:   # sent_ids contain the ids of all the sentences that make up the training instance represented by the dict: contg_psg_dict
                            ...
                            for mtchprot in matched_prots:  # There may be 2 or more matched proteins "mtchprot" for the same DatumKB protein "prot" in a single sentence
                                if mtchprot in depfeat_data[pmid][sentid]:
                                    ...
                                    article_level_features.extend(depfeat_data[pmid][sentid][mtchprot].keys())
