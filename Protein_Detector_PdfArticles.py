...
# python Protein_Detector_PdfArticles.py ./Minimized_Protein_Names_FST.fst ./SymbolTable_Protein_Names_FST.sym 19 ./Total_PubMedIds_John_NoOpenAccess.txt         ##### ./PubMedIDS_with_Extras.txt

def __main__():
    parser = argparse.ArgumentParser(prog='Protein_Detector', usage='Protein_Detector_PdfArticles.py <FST file> <SymbolTable file> <Maximum Length of a protein name> <PubMedfilelist> [--file <Optional Filename to be parsed>]', description='Script to read the segmented sentences and recognize protein names')
...
def read_dirs(fst_file, symboltable_file, max_prot_len, PubMedfilelist):
    global my_fst; global syms; global badfile_bit; global stopwords;
    ...
    rootDir = '/ua/ml-group/big-mechanism-project/PLEIADES/Sep2016_SMA/JSON_SENTENCE_ASSOCS'
    writeDir = '/ua/ml-group/big-mechanism-project/PLEIADES/Sep2016_SMA/protein_detect_output'
    ...
    with open(PubMedfilelist, 'rt') as f1:
        for pmid in f1:
            ...
            for itm in myfile_data["sentenceList"]["@items"]:  # itm is a dictionary. It represent one line of text in the file 
                ...
                matches = []; ctr = 0; old_text = ""; new_text = itm["sentenceText"].replace("\n", " ").replace("\r", "")
                ...
                while len(matches) == 0 and ctr <= 3:
                    ...
                    if ctr == 1:
                        if "-" in itm["sentenceText"]:
                            new_text = itm["sentenceText"].replace("-", " ")
                    if ctr == 2:
                        if "-" in itm["sentenceText"]:
                            new_text = itm["sentenceText"].replace("-", "")
                    if ctr == 3:
