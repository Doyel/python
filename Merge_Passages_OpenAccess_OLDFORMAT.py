...
# python Merge_Passages_OpenAccess_OLDFORMAT.py RNAi
# python Merge_Passages_OpenAccess_OLDFORMAT.py RNAi --feedback ./user_feedback_JSON/Apr4_2017/openaccess_passage_dict_WithFeedback.json
        ...
        for pmid in myprotdict:
            ...
            for prot in myprotdict[pmid][label]:
                if args.feedback == None:
                    ...
                    if myclass == "Neg" and prot not in passage_dict[pmid][label][myclass]:
                        passage_dict[pmid][label][myclass][prot] = {}   # During feedback processing, a positive protein will not exist in negprotdict
                passage_dict[pmid][label][myclass][prot]["passageDetails"] = []
                absIds = myprotdict[pmid][label][prot].keys()
                grouped_absIds = group_absoluteIds(absIds)    # Returns a sorted list of contiguous group of absolute id lists
                grouped_partial_absIds = group_partial_absoluteIds(grouped_absIds, myprotdict[pmid][label][prot])  # Returns a list of list of tuples
                psg_ctr = 0; foundprot = prot.replace(" ", "__")
                for contg_grp in grouped_partial_absIds:    # contg_grp is a list of tuples
                    tempdict = {}
                    passage_text = get_passagetext(contg_grp, myprotdict[pmid][label][prot])
                    ...
                    for sameparagrp in sameparagrpids:  # sameparagrp is a list of tuples
                        tempdetdict = {}
                        same_passage_text = get_passagetext(sameparagrp, myprotdict[pmid][label][prot])
                        ...
                        for idx in sameparagrp:     # idx is a tuple
                            tempdetdict["sentenceDetails"].append(myprotdict[pmid][label][prot][idx[0]][idx[1]])
