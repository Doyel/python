...
# python Process_UserFeedback.py RNAi ./user_feedback_JSON/Apr4_2017/SavedHighlights_WithFeedback.json ./downloaded_articles ./JSON_SENTENCE_ASSOCS_OpenAccess ./datafiles/datafiles_NoMutualExc_NoProtFeat_NoFreqPrune_Neighborhood0
...
def main_body():
    ...
    for pmid in feedback_data:
        ...
        for prot in feedback_data[pmid][label]:
            ...
            for span in feedback_data[pmid][label][prot]["HighlightObject"]:    # "HighlightObject" is a list of lists. 'span' is a list with 5 elements
                ...
                #if pmid == "20026654" and prot == "nbs1":
                #    pprint(tempdict); print idxpathlist
    ...
    for pmid in span_list:
        ...
        for prot in span_list[pmid]:
            ...
            posprotdict[pmid][label][prot]["AbsoluteIds"] = {}  # The interface returns the WHOLE set of evidence sentences for any selected combination of - 'prot' and 'label'
            negprotdict[pmid][label][prot]["AbsoluteIds"] = {}  # 'prot' wouldn't exist in negprotdict beforehand as it is a positive protein
                        ...
                        for each_psg in contg_passages:  # contg_passages is a list of dicts
                            ...
                            for each_sent in sent_details:  # sent_details is a list of dicts
                                ...
                                if color == "Yellow":
                                    ...
                                    if "partialSentence" in each_sent and not checkProtein(each_sent):  # Call checkProtein for only those partial sentences that were unhighlighted
                                        continue                                                        # by user for a protein that was given feedback on
                                    each_sent["HasProtein"] = "Yes"
                                    if each_sent["absoluteId"] in negprotdict[pmid][label][prot]["AbsoluteIds"]:
                                        negprotdict[pmid][label][prot]["AbsoluteIds"][each_sent["absoluteId"]].append(each_sent)
                                    else:
                                        negprotdict[pmid][label][prot]["AbsoluteIds"][each_sent["absoluteId"]] = [each_sent]
            ...
            if not negprotdict[pmid][label][prot]["AbsoluteIds"]:
                ...
                for absid in negprotdict[pmid][label][prot]["AbsoluteIds"]:
                    if len(negprotdict[pmid][label][prot]["AbsoluteIds"][absid]) > 1:
                        newlist = sorted(negprotdict[pmid][label][prot]["AbsoluteIds"][absid], key=itemgetter('partialOffset'))
...
def highlightnode(pmid, prot, copy_soup, spandict, mydict):
    ...
    #print spandict["spanPath"], "\t", childidxpath, "\t", spandict["idx"]
    #print getnodefromidxpath(spandict["spanPath"], copy_soup.body)
...
def getContiguousPassages(paraDetails, hilitednode, color):
    #print "Inside getContiguousPassages"
    mylist = []
    ...
    #print "Return from getContiguousPassages"
    return mylist
...
def getSentenceList(origList, psg):
    ...
    #print "Returning from getSentenceList"
    #pprint(sentenceList)
    return sentenceList
